...
void id_number(){
	...
	while (vc == 1){
		float num;
        system("cls"); Sleep(300);
		printf ("\nIngrese el numero a evaluar: ");
		scanf ("%f", &num); Sleep(600);
		if (num>0) {  printf ("\nEl numero es positivo");Sleep(800); }

		else
		if (num<0) {  printf ("\nEl numero es negativo");Sleep(800); }

		else
		if (num==0) {  printf ("\nEl numero es elemento neutro");Sleep(800);}
...
void ParesSP(){
    ...
    printf("Los pares son: ");Sleep(1000);

    while (x >= 20 && x <= 40){
        if (x%2 == 0){
            printf("%d, ", x);Sleep(250);
            suma += x;
            producto *= x;    
        }
        x ++;
    ...
    Sleep(500);
    printf("\nLa suma de los numeros es: %d\nEl producto de los numeros es: %lld", suma, producto);Sleep(1000);
...
void HospitalPresupuesto(){
    ...
    while (vc == 1){
        ...
        printf("Desea trabajar con Dolares /$/ o bolivares /bs.s/?\n$ = 1\nbs.s = 2\n"); scanf("%d", &moneda);
        ...
        Sleep(800);
        printf("\nIntroduzca el monto presupuestal: ");scanf("%d", &presupuesto);
        ...
        Sleep(600);
        printf("\nSe ha repartido el presupuesto insertado de la siguiente manera:\n\n");Sleep(600);
        if (moneda == 1){printf("Ginecologia = %d %c\nTraumatologia = %d %c\nPediatria = %d %c\n", Gine, dolar, Trauma, dolar, Pedi, dolar);}
        if (moneda == 2){printf("Ginecologia = %d%s\nTraumatologia = %d%s\nPediatria = %d%s\n", Gine, mony, Trauma, mony, Pedi, mony);}
        Sleep(600);
...
int main(){
    ...
    while(vc == 1){
        ...
        if (opcion == 5){ printf("\nFin del programa."); Sleep(1300); return 0;}
        while (opcion < 1 || opcion > 5){
            printf("\nPor favor ingrese una opcion valida: ");scanf("%d", &opcion);
    ...
    if (vc != 1){ printf("\nFin del programa."); Sleep(1500); return 0;}
